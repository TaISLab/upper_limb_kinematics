...
def normal_unitario_2D(v, sentido='izquierda'):
    """
    Devuelve el vector normal unitario a un vector 2D.

    Parámetros:
        v : iterable (x, y)
        sentido : 'izquierda' o 'derecha' (dirección de la rotación 90°)

    Retorna:
        np.array con la normal unitaria
    """
...
def offset_polygon_2d_rco(vertices, espesor):
        """
        Offset (inset/outset) de un polígono 2D por intersección de aristas desplazadas.
        - espesor > 0: hacia el interior (adelgaza)
        - espesor < 0: hacia el exterior (expande)
        Devuelve una lista de (x, y) con la misma orientación que la entrada.
        """

        # Obtener los vectores entre los puntos. Sentido Horario
        V = [(float(x), float(y)) for x, y in vertices]
        V = [(x, y) for x, y in V]
        # Calcular las normales
        normales = [normal_unitario_2D((x2 - x1, y2 - y1), sentido='derecha') for (x1, y1), (x2, y2) in zip(V, V[1:] + [V[0]])]
        # Aplicar el offset
        V_offset = [(x + espesor * nx, y + espesor * ny) for (x, y), (nx, ny) in zip(V, normales)]
        return V_offset
...
def max_inscribed_circle_in_polygon(points_xy):
    """
    Calcula el círculo de mayor radio inscrito en el polígono convexo definido por points_xy.
    Devuelve el centro (c) y el radio (r).
    """
    hull = convex_hull(points_xy)
    if len(hull) < 3:
        raise ValueError("Se necesitan al menos 3 puntos no colineales.")
    ...
    for i in range(A.shape[0]):
        ...
        b_i = b[i]
        constraints += [
            a_i @ c + r * np.linalg.norm(a_i) <= b_i
        ]
    ...
    return c.value, r.value, hull, prob.status
# -----------------------------
...
def draw_overlay(marker_pub, hull, c, G, x_cota=0.0):
    ...
    # blanco
    diag_marker.color.r = 1.0
    diag_marker.color.g = 1.0
...
class EllipseMethodNode:
    def __init__(self):
        rospy.init_node('ellipse_method_4p_node')
        rospy.loginfo("Ellipse Method Node started.")
        self.rate = rospy.Rate(5)  # 10 Hz

        # Publisher para los puntos en RViz
        self.marker_pub = rospy.Publisher('/ellipse_4p_vertices_marker', Marker, queue_size=1)
        self.overlay_pub = rospy.Publisher('/ellipse_4p_overlay_marker', Marker, queue_size=1)
        self.angle_pub = rospy.Publisher('/ellipse_4p_angle_deg', AngleStamped, queue_size=1)
        self.centro_elipse_pub = rospy.Publisher('/ellipse_4p_centro', PointStamped, queue_size=1)
    ...
    def publish_centro_point(self, centro, frame_id="base_gripper"):
        from geometry_msgs.msg import PointStamped
        msg = PointStamped()
        msg.header.stamp = rospy.Time.now()
        msg.header.frame_id = frame_id
        msg.point.x = float(self.x_cota)  # Usa el valor de x_cota correspondiente
        msg.point.y = float(centro[0])
        msg.point.z = float(centro[1])
        self.centro_elipse_pub.publish(msg)

    def calculate_ellipse_from_vertices(self, vertices):
        """
        Calcula el círculo de mayor radio inscrito en el polígono.
        """
        x = vertices[0][0]
        vertices = [(float(v[1]), float(v[2])) for v in vertices]

        c, r, hull, status = max_inscribed_circle_in_polygon(vertices)
        if status not in ("optimal", "optimal_inaccurate") or c is None or r is None:
            rospy.logwarn("No se pudo calcular un círculo válido.")
            return {
        ...
        # Para dibujar la circunferencia como elipse con G = r * I
        G = np.eye(2) * r
        angle_deg = 0.0  # No hay orientación en un círculo
        ...
        return {
            ...
            "semi_eje_mayor_a_px": float(r),
            "semi_eje_menor_b_px": float(r),
            "angulo_eje_mayor_grados": float(angle_deg),
            "poligono_vertices": vertices,
            "elipse_vertices": elipse_pts_with_x,
        }
    ...
    def obtener_vertices_elipse(self):
        ...
        # Obten el valor de TF entre los frames: base_gripper y dedo1_link1 y base_gripper y dedo1_link2
        
        try:
            t_d41 = self.tfBuffer.lookup_transform('base_gripper', 'dedo1_link1', rospy.Time(0), rospy.Duration(1.0))
            t_d42 = self.tfBuffer.lookup_transform('base_gripper', 'dedo1_link2', rospy.Time(0), rospy.Duration(1.0))
            t_d31 = self.tfBuffer.lookup_transform('base_gripper', 'dedo2_link1', rospy.Time(0), rospy.Duration(1.0))
            t_d32 = self.tfBuffer.lookup_transform('base_gripper', 'dedo2_link2', rospy.Time(0), rospy.Duration(1.0))
            ...
            p_d41 = np.array([t_d41.transform.translation.x, t_d41.transform.translation.y, t_d41.transform.translation.z])
            vector_x_d41 = R_d41[:, 0]  # Eje x del dedo1_link1
            self.vector_x_d41 = vector_x_d41

            p_d42 = np.array([t_d42.transform.translation.x, t_d42.transform.translation.y, t_d42.transform.translation.z])
            vector_x_d42 = R_d42[:, 0]  # Eje x del dedo1_link2
            self.vector_x_d42 = vector_x_d42

            # Dedo 3
            p_d31 = np.array([t_d31.transform.translation.x, t_d31.transform.translation.y, t_d31.transform.translation.z])
            vector_x_d31 = R_d31[:, 0]  # Eje x del dedo2_link1
            self.vector_x_d31 = vector_x_d31

            p_d32 = np.array([t_d32.transform.translation.x, t_d32.transform.translation.y, t_d32.transform.translation.z])
            vector_x_d32 = R_d32[:, 0]  # Eje x del dedo2_link2
            self.vector_x_d32 = vector_x_d32
            ...
            # Calcular P:
            P1 = p_d42
            P2 = self.intersection_of_lines(p_d42, vector_x_d42, p_d32, vector_x_d32)
            P3 = p_d32
            P4 = self.intersection_of_lines(p_d42, -vector_x_d41, p_d32, -vector_x_d31)


            # rospy.loginfo(f"Vector x dedo1_link2: {vector_x_d42}, Vector x dedo2_link2: {vector_x_d32}")

            # Publicar vectores en RViz
            # self.publish_vectors_marker([p_d42, p_d32], [vector_x_d42, vector_x_d32], ns="finger_vectors", color=(0.2, 0.2, 1.0), frame_id="base_gripper")

            if P2 is not None and P3 is not None:
                return P1, P2, P3, P4
            else:
                return P1, None, P3, None
    ...
    def publish_vertices_marker(self, vertices, ns="ellipse_vertices", id=0):
        ...
        if vertices is None or len(vertices) != 4:
            rospy.logwarn("publish_vertices_marker: Se requieren exactamente 4 vértices (P1, P2, P3, P4).")
            return
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            P1, P2, P3, P4 = self.obtener_vertices_elipse()
            ...
            vertices_2d = [P1[1:], P2[1:], P3[1:], P4[1:]] # Plano YZ
            x_cota = P1[0]

            

            vertices_2d = [(P1[1], P1[2]), 
                        (P2[1], P2[2]), 
                        (P3[1], P3[2]), 
                        (P4[1], P4[2])]

            # rospy.loginfo(f"Vértices del polígono original (YZ): {vertices_2d}")
            ...
            # rospy.loginfo(f"vertices_inset_3d: {vertices_inset_3d}")
            # rospy.loginfo(f"P2, P4 ajustado tras adelgazamiento")
            self.publish_vertices_marker(vertices_inset_3d, ns="inset_polygon_adjusted", id=2)
            ...
            self.x_cota = x_cota  # Guarda x_cota para usar en publish_centro_point
            self.publish_centro_point(centro, frame_id="base_gripper")
            ...
            tiempo_total = (rospy.Time.now() - t0).to_sec()

            # Haz un rospy.loginfo en una linea con los tiempos de cálculo
            rospy.loginfo(f"Tiempos (ms): Offset={tiempo_offset*1000:.2f}, Elipse={tiempo_elipse*1000:.2f}, Total={tiempo_total*1000:.2f}")
